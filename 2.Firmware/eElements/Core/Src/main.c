...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    ...
    EPD_ShowChinese(20, 0, 3, 32, BLACK);  // ╣Г
    EPD_ShowChinese(52, 0, 4, 32, BLACK);  // вс
    EPD_ShowChinese(84, 0, 5, 32, BLACK);  // ж╫
    EPD_ShowChinese(116, 0, 6, 32, BLACK); // ╡Б
    EPD_ShowChinese(148, 0, 7, 32, BLACK); // йт
    ...
    {
        printf("%d\t%s\t%s\t%s\r\n",
               elementInfo[i].atomicNumber,
               elementInfo[i].nameCN,
               elementInfo[i].nameEN,
               elementInfo[i].atomicSymbol);
        HAL_Delay(100);
    }
    */
